...
class UsersQueryRepository {
  ...
  async getDBUser(param: string) {
    ...
    if (ObjectId.isValid(param)) {
      user = await UserModel.findOne({ _id: new ObjectID(param) });
    } else if (param.indexOf('@') !== -1) {
      user = await UserModel.findOne({ 'accountData.email': param });
    } else {
      user = await UserModel.findOne({ 'accountData.login': param });
    };
