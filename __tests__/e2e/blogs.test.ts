...
describe('/hometask_05/api/blogs', () => {
  ...
  it('READ blog1. Status 200', async () => {
    const response = await request(app)
      .get(`/hometask_05/api/blogs/${blog1.id}`)
  ...
  it('UPDATE blog1 (unauthorized). Status 401', async () => {
    await request(app)
      .put(`/hometask_05/api/blogs/${blog1.id}`)
      .auth('admin', 'admin', { type: 'basic' })
      .send(reqBodyToUpdate)
      .expect(HTTP.UNAUTHORIZED_401);
  }); // blogs = [blog1];
  ...
  it('GET posts of blog 100. Status 404', async () => {
    await request(app)
      .get(`/hometask_05/api/blogs/100/posts`)
      .expect(HTTP.NOT_FOUND_404);
  });
  ...
  it('GET posts of blog1. Status 200', async () => {
    ...
    await request(app)
      .get(`/hometask_05/api/blogs/${blog1.id}/posts`)
      .expect(HTTP.OK_200, {
        pagesCount: 0,
        page: 1,
        pageSize: 10,
  ...
  it('POST post for blog100. Status 404', async () => {
    await request(app)
      .post(`/hometask_05/api/blogs/100/posts`)
      .auth('admin', 'qwerty', { type: 'basic' })
  ...
  it('POST post for blog1 (unauthorized). Status 401', async () => {
    await request(app)
      .post(`/hometask_05/api/blogs/${blog1.id}/posts`)
      .auth('admin', 'admin', { type: 'basic' })
      .send(reqBodyToCreatePost)
      .expect(HTTP.UNAUTHORIZED_401);
  });
  ...
  it('Delete blog with id 100. Status 404', async () => {
    await request(app)
      .delete(`/hometask_05/api/blogs/100`)
      .auth('admin', 'qwerty', { type: 'basic' })
      .expect(HTTP.NOT_FOUND_404);
  }); // blogs = [blog1];
  ...
  it('Delete blog1 (unauthorized). Status 401', async () => {
    await request(app)
      .delete(`/hometask_05/api/blogs/${blog1.id}`)
      .auth('admin', 'admin', { type: 'basic' })
      .expect(HTTP.UNAUTHORIZED_401);
  }); // blogs = [blog1];
